...
const Home = ({ darkMode, toggleTheme }) => {
  ...
  const handleAnswerSubmit = () => {
    if (!hasAttempted) {
      ...
      const today = new Date().toDateString();
      const dailyChallengeKey = `dailyChallenge_${profile?.email}_${today}`;
      ...
      if (selectedAnswer === dailyChallenge.question.correctAnswer) {
        const userHistoryKey = `quizHistory_${profile?.email}`;
  ...
  useEffect(() => {
    if (quizHistory && quizHistory.length > 0) {
      const stats = {
        ...
        bestScore: Math.max(...quizHistory.map(q => (q.score / q.totalQuestions) * 100)),
        totalTimeSpent: quizHistory.reduce((total, quiz) => total + quiz.timeSpent, 0),
        topicsMastered: aptitudeTopics.filter(topic => {
          const topicQuizzes = quizHistory.filter(q => q.topic === topic);
          return topicQuizzes.length > 0 && 
                 topicQuizzes.every(q => (q.score / q.totalQuestions) * 100 >= 70);
        }).length
      };
      setProfileStats(stats);
    }
  }, [quizHistory, aptitudeTopics, calculateAverage]);
        ...
        {profile && (
          <Grid container spacing={3} sx={{ mb: 4 }}>
            ...
            <Grid item xs={12} md={4}>
              <Paper elevation={0} sx={{ p: 3, borderRadius: 4, minHeight: 320, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#f8fafd', border: '1px solid #e0e7ef', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                <Typography variant="h6" sx={{ fontWeight: 'bold', mb: 2, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <MilitaryTechIcon sx={{ mr: 1, color: '#e53935' }} />
                  Your Achievements
                </Typography>
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mb: 2 }}>
                  <Box sx={{ width: 140, height: 140, borderRadius: '50%', background: '#f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'center', mb: 2, border: '2px solid #e0e7ef' }}>
                    <Typography variant="h2" sx={{ fontWeight: 'bold', color: '#e53935' }}>{calculateAverage()}%</Typography>
                  </Box>
                  <Typography variant="h6" sx={{ fontWeight: 'bold', mb: 1 }}>{userLevel.level} Level</Typography>
                  <Typography variant="body1" sx={{ mb: 1 }}>Completed {quizHistory.length} test{quizHistory.length !== 1 ? 's' : ''}</Typography>
                  <Chip icon={<CheckCircleIcon />} label={quizHistory.length >= 5 ? "Regular Tester" : "Taking Progress Steps"} color="primary" variant="outlined" size="medium" sx={{ borderRadius: 2, fontWeight: 'bold' }} />
                </Box>
              </Paper>
            </Grid>
          </Grid>
